...
import { getRelatedPosts as getRelatedPostsIT } from '@/lib/blog-client-it';
import { BLOG_CONFIG } from '@/lib/constants';
import type { RelatedPost } from '@/types/blog';

interface RelatedPostsITProps {
...
export function RelatedPostsIT({ currentPostId, categories, tags, variant = 'grid' }: RelatedPostsITProps) {
  ...
  useEffect(() => {
    async function loadRelatedPosts() {
      try {
        const posts = await getRelatedPostsIT(
          currentPostId,
          categories,
          tags,
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Continua a esplorare approfondimenti sull'automazione aziendale e la trasformazione digitale
          </p>
        </div>

        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {relatedPosts.map((post) => (
            <RelatedPostCardIT key={post.id} post={post} />
          ))}
        </div>
      </div>
    </section>
