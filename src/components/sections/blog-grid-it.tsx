...
import { getBlogPosts as getBlogPostsIT, getCategories as getCategoriesIT } from '@/lib/blog-client-it';
import { BLOG_CONFIG } from '@/lib/constants';
import type { BlogPost, Category, BlogPostsResponse } from '@/types/blog';

interface BlogGridITProps {
  page?: number;
  category?: string;
  tag?: string;
  search?: string;
...
export function BlogGridIT({ page = 1, category, tag, search }: BlogGridITProps) {
  ...
  const handleSearch = (query: string) => {
    ...
    router.push(`/it/blog?${params.toString()}`);
  };
  ...
  if (!blogData) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">Impossibile caricare gli articoli del blog.</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {blogData.total > 0 ? (
            <>
              Mostrando {((page - 1) * BLOG_CONFIG.POSTS_PER_PAGE) + 1} a{' '}
              {Math.min(page * BLOG_CONFIG.POSTS_PER_PAGE, blogData.total)} di{' '}
              {blogData.total} articoli
            </>
          ) : (
      ...
      {blogData.posts.length > 0 ? (
        <>
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {blogData.posts.map((post) => (
              <BlogPostCardIT key={post.id} post={post} />
            ))}
          </div>
        ...
        <div className="text-center py-12">
          <p className="text-muted-foreground mb-4">
            Nessun articolo trovato che corrisponda ai tuoi criteri.
          </p>
          <Button onClick={() => router.push('/it/blog')}>
...
function BlogPostCardIT({ post }: { post: BlogPost }) {
  return (
