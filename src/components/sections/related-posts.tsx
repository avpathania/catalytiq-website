...
import { getRelatedPosts } from '@/lib/blog-client';
...
export function RelatedPosts({ currentPostId, categories, tags, variant = 'grid' }: RelatedPostsProps) {
      ...
      <aside className="space-y-6">
        ...
        <div className="flex flex-col gap-4">
          {relatedPosts.map((post) => (
            <RelatedPostCard key={post.id} post={post} compact />
          ))}
        </div>
      </aside>
    );
...
function RelatedPostCard({ post, compact = false }: { post: RelatedPost; compact?: boolean }) {
  if (compact) {
    return (
      <Card className="overflow-hidden border-0 bg-card/50 backdrop-blur-sm hover:bg-card/70 transition-colors group">
        <Link href={`/blog/${post.slug}`} className="flex gap-3 p-3">
