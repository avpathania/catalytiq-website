...
interface BlogPostITProps {
  post: BlogPostType;
}

export function BlogPostIT({ post }: BlogPostITProps) {
  return (
    <div className="py-8">
      <div>
        <div>
          ...
          <div className="mb-8">
            <Button variant="ghost" asChild>
              <Link href="/it/blog" className="flex items-center gap-2">
          ...
          <header className="text-center mb-12">
            ...
            <div className="flex flex-wrap justify-center gap-2 mb-6">
              {post.categories?.map((category) => (
                ...
                >
                  <Link href={`/it/blog?category=${category.slug}`}>
                    {category.name}
                  </Link>
                </Badge>
              ))}
            ...
            <div className="flex flex-wrap items-center justify-center gap-6 text-muted-foreground">
              ...
              {post.view_count > 0 && (
                <div className="flex items-center gap-2">
                  <span>{post.view_count} visualizzazioni</span>
                </div>
              )}
            </div>
          </header>
              ...
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <Badge key={tag.id} variant="secondary">
                    <Link href={`/it/blog?tag=${tag.slug}`}>
                      {tag.name}
                    </Link>
                  </Badge>
                ))}
              ...
              <div className="flex gap-6">
                ...
                <div className="flex-1">
                  <h3 className="text-xl font-semibold mb-2">
                    Informazioni su {post.author.name}
                  </h3>
                  {post.author.bio && (
