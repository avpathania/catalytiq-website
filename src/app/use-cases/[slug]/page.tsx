...
import { MainLayout, UseCasesCTA } from "@/components";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!study) {
    return { title: "Case Study Not Found | CatalytIQ Systems" };
  }
...
export default async function CaseStudyPage({ params }: Props) {
  ...
  const [study, related] = await Promise.all([
    getCaseStudy(slug),
    getCaseStudy(slug).then((s) =>
      s ? getRelatedCaseStudies(slug, s.industry) : []
    ),
  ]);
    ...
    <MainLayout>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-3xl">
              <div className="flex items-center gap-3 p-4 rounded-xl bg-muted/50 border">
                ...
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium mb-0.5">Client Profile</p>
          ...
          <div className="mx-auto max-w-3xl">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              ...
              <div className="flex items-start gap-4 p-4 rounded-xl bg-muted/50 border">
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium mb-1">Industry</p>
                  <p className="text-sm font-semibold">{study.industry}</p>
                </div>
              </div>
              <div className="flex items-start gap-4 p-4 rounded-xl bg-muted/50 border">
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium mb-1">Automation Type</p>
              ...
              <div className={`grid grid-cols-1 gap-4 ${study.timeline && study.investment ? "sm:grid-cols-2" : ""}`}>
                {study.timeline && (
                  <div className="p-5 rounded-xl bg-muted/50 border">
                    <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium mb-1">Timeline</p>
                    <p className="text-lg font-bold">{study.timeline}</p>
                  </div>
                )}
                {study.investment && (
                  <div className="p-5 rounded-xl bg-primary/5 border border-primary/10">
                    <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium mb-1">Investment</p>
      ...
      {related.length > 0 && (
        <section className="py-16 border-t bg-muted/30">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-3xl">
              <h2 className="text-xl font-bold mb-8">More Case Studies</h2>
